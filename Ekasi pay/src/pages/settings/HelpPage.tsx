...
export const HelpPage = ({ navigate }: { navigate: (p: string) => void }) => {
  ...
  const handleWhatsApp = () => {
    if (!support.whatsappE164) {
      toast.error('WhatsApp support is not configured for this build.');
      return;
    }
    toast.info('Opening WhatsApp...');
    window.open(
      `https://wa.me/${support.whatsappE164}?text=${encodeURIComponent('Hi KasiPay Support')}`,
      '_blank',
    );
  };
  ...
  const handleCall = () => {
    if (!support.phoneTel) {
      toast.error('Call-center number is not configured for this build.');
      return;
    }
    toast.info('Calling...');
    window.location.href = `tel:${support.phoneTel}`;
  };

  const handleEmail = () => {
    if (!support.email) {
      toast.error('Support email is not configured for this build.');
      return;
    }
    ...
    <PageTransition className="min-h-0 h-full bg-slate-50">
      ...
      <div className="flex-1 min-h-0 overflow-y-auto p-6 pb-8 space-y-8">
        ...
        <section>
          ...
          <KPCard className="p-5 space-y-3 text-sm text-slate-700 leading-relaxed">
            <p>
              ...
              <em className="not-italic text-emerald-700">Shop</em>, tap products, then
              checkout — cash or customer wallet (capture their phone when needed).
            </p>
            <p>
            ...
            <p>
              ...
              <em className="not-italic text-emerald-700">Services</em> → capture KYC
              honestly; beneficiary ID must match the document scanned at payout. Use
              the camera scanner icon or a USB wedge pointed at ID barcodes.
            </p>
            <p>
              <strong className="text-slate-900">Scanner tips:</strong> add light, hold
              steady ~20–40 cm away; if the camera fails, plug in a retail USB scanner
              and focus the text field — digits type in automatically.
            </p>
            <p>
        ...
        <div className="text-center pt-8 pb-4">
          <p className="text-xs text-slate-400 font-medium">KasiPay Spaza App v2.1.0</p>

    ...
    if (previous !== false) {
      await client.query(
        `UPDATE operational_controls
            SET enabled = FALSE, version = version + 1, reason = $1,
                changed_by = 'system:recon-guard', changed_at = clock_timestamp()
      ...
      await client.query(
        `INSERT INTO operational_control_events
         ...
         VALUES ($1,'financial_posting',$2,FALSE,$3,'system:recon-guard',$4)`,
...
export async function disablePostingOnLedgerDriftPg(
  ...
  const result = await disableFinancialPostingPg(database, reason);
